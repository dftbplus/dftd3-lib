...
program test_libdftd3
  ...
  ! Set input parameters for the library
  input%threebody = .not. noabc
  input%numgrad = numgrad
  if (minc6) then
    allocate(input%minc6list(max_elem))
    input%minc6list(:) = minc6list
  end if
  if (maxc6) then
    allocate(input%maxc6list(max_elem))
    input%maxc6list(:) = maxc6list
  end if
  input%cutoff = sqrt(rthr)
  input%cutoff_cn = sqrt(rthr2)
  call dftd3_init(mydftd3, input)
  ...
  if (ex) then
    ...
    call dftd3_set_functional(mydftd3, func, version, tz)
  else
    call rdpar(dtmp,version,s6,s18,rs6,rs18,alp)
    call dftd3_set_params(mydftd3, [s6, rs6, s18, rs18, alp], version)
  end if
  ...
  if (pbc) then
    if (grad) then
      call dftd3_pbc_dispersion(mydftd3, xyz, iz, lat, disp, g, g_lat)
    else
      call dftd3_pbc_dispersion(mydftd3, xyz, iz, lat, disp)
    end if
    ...
    if (grad) then
      call dftd3_dispersion(mydftd3, xyz, iz, disp, g)
    else
      call dftd3_dispersion(mydftd3, xyz, iz, disp, g)
    end if
    !!call edisp(max_elem,maxc,n,xyz,iz,c6ab,mxc,r2r4,r0ab,rcov, &
